...
import { AiTextError, generateVerifiedPetContent, getTextModelName } from "@/services/ai/text";
...
export async function POST() {
  ...
  try {
    ...
    const published = await prisma.petContentGeneration.findMany({ where: { productId: product.id, status: "PUBLISHED" }, select: { content: true }, orderBy: { createdAt: "desc" }, take: 10 });
    const facts = {
    ...
    const result = await generateVerifiedPetContent(facts, history.map((item) => item.topic), published.map((item) => item.content));
    ...
    if (error instanceof AiTextError) return fail(error.code, error.message, 502);
