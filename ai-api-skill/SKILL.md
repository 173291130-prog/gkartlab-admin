...
3. 如果都找不到，提示用户提供 API Key，并引导用户前往      OVOAI 官网（https://ovoai.net）登录后在「API 密钥管理」中创建
...
部分模型需要传入图片、视频、音频等文件（参数 type=upload），有两种传法：

**方式一（本地文件直接传 base64）**：图片（jpeg/png/webp/gif）和音频（mp3/wav/m4a/ogg）可把参数值直接写成 data:<mime>;base64,<数据>，服务端自动转存；限制：单文件解码后 ≤10MB、单次合计 ≤30MB、请求体总 ≤50MB；视频不支持 base64。余额不足会在转存前拦截（code 402）。
...
3. **余额不足时**，提示用户前往      OVOAI 官网（https://ovoai.net）充值算力
4. **API Key 错误时**，提示用户前往      OVOAI 官网（https://ovoai.net）登录并在「API 密钥管理」中创建或检查密钥
